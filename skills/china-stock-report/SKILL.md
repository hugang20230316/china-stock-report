...
7. 先对首只正式入选股票执行一次单股截图预检；若 `playwright`、浏览器或东方财富页面在 5 分钟内不可用，立即停止并汇报失败点，不要继续空跑整批。
8. 运行 `node scripts/screenshot.js --date YYYYMMDD --concurrency 3 --stocks '[...]'`，为正式入选股票重新抓取当天 K 线截图。
9. 运行 `node scripts/generate_report_html.js --date YYYYMMDD`，生成正式 HTML。
10. 运行 `node scripts/capture_report_preview.js --date YYYYMMDD`，生成可直接查看的成品预览 PNG。
11. 回复用户时默认只输出 4 件事：最终 HTML 路径、预览 PNG 路径、实际使用了哪些 skill、以及一句简要说明。

## 强制规则

- 所有行情、涨幅、估值、截图时间都必须来自本次实际执行结果，并明确绝对日期。
- 默认只接受 `stock_report_YYYYMMDD.html` 作为正式交付物；JSON、Markdown、临时脚本输出只能作为中间产物。
- 默认还要同时给出 `assets/YYYYMMDD/report_preview.png` 作为成品预览，避免用户只拿到文件路径却看不到页面效果。
...
- 会优先加载当前环境的 `playwright`，加载不到时会继续尝试 `STOCK_PLAYWRIGHT_PATH`、工作区 `node_modules/playwright`、本机常见缓存路径，并自动探测 Chrome/Edge 可执行文件。
...
### `scripts/capture_report_preview.js`

- 使用本机 Chrome/Edge 无头模式，把正式 HTML 输出成 PNG 预览图。
- 默认输出到 `assets/YYYYMMDD/report_preview.png`，也可手动指定输出路径。
